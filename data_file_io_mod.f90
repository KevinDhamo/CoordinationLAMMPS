...
contains
    ...
    ! New function that only reads atom count and box dimensions
! Improved function that reads atom count and box dimensions correctly
    subroutine read_data_file_dims_only(filename, n_atoms, n_types, box_length)
        ...
        do
            ...
            ! Debug output for box dimension lines
            if (VERBOSE .and. (index(line, 'lo') > 0 .and. index(line, 'hi') > 0)) then
                write(*,*) "Box dimension line found: ", trim(line)
            end if
            ...
            if (index(line, 'atoms') > 0 .and. index(line, 'atom types') == 0) then
                ...
                if (io_stat == 0) then
                    box_length(1) = xhi - xlo
                    found_box_x = .true.
                    if (VERBOSE) write(*,*) "Found X box: ", xlo, xhi, box_length(1)
                end if
            else if (index(line, 'ylo yhi') > 0) then
                read(line, *, iostat=io_stat) ylo, yhi
                if (io_stat == 0) then
                    box_length(2) = yhi - ylo
                    found_box_y = .true.
                    if (VERBOSE) write(*,*) "Found Y box: ", ylo, yhi, box_length(2)
                end if
            else if (index(line, 'zlo zhi') > 0) then
                read(line, *, iostat=io_stat) zlo, zhi
                if (io_stat == 0) then
                    box_length(3) = zhi - zlo
                    found_box_z = .true.
                    if (VERBOSE) write(*,*) "Found Z box: ", zlo, zhi, box_length(3)
                end if
            end if
